...
function AdminPanel() {
  ...
  const validarFormulario = () => {
    ...
    if (formData.imagenUrl.trim()) {
      ...
      if (!urlRegex.test(formData.imagenUrl))
        return "La URL de la imagen no es válida. Debe terminar en jpg, jpeg, png o webp.";
    }
    return null;
  };
  ...
  const handleConfirmDelete = async () => {
    if (confirmDelete.producto) {
      ...
      mostrarAlerta(
        `Camiseta "${confirmDelete.producto.nombre}" eliminada`,
        "danger",
      );
      cargarProductos();
    }
  ...
  return (
    <Container className="py-4 admin-panel-container">
      <center>
        <Alert
          variant="light"
          className="shadow-sm p-4 border rounded-4 admin-header text-center"
        >
          <h4 className="fw-bold mb-0">Bienvenido, Administrador</h4>
        </Alert>
      ...
      {alert.show && (
        <Alert
          ...
          className="rounded-3 shadow-sm text-center"
        >
          {alert.message}
        </Alert>
      )}
                              ...
                              <tr key={producto.id}>
                                ...
                                <td className="text-nowrap">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => openConfirmDelete(producto)}
                                  >
                                    Eliminar
                                  </Button>
                                </td>
